...
def encrypt_decrypt():
    ...
    return render_template('index.html', input_text=input_text, key=key, mode=mode, input_type=input_type, output_text=output_text)
...
def middle_attack():
    found_keys = None
    if request.method == 'POST':
        known_plain_texts = request.form.getlist('known_plain_texts')
        known_ciphers = request.form.getlist('known_ciphers')

        # Perform meet-in-the-middle attack
        found_keys = meet_in_the_middle_attack(known_plain_texts, known_ciphers)

    return render_template('middle_attack.html', found_keys=found_keys)
